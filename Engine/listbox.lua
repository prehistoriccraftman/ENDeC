...
local listbox = {
	x = 100,
	y = 100, -- listbox position x,y (number)
	w = 200,
	h = 400, -- listbox size Width x Height (number)
	hor = true,
	ver = true, -- Show scroller Horizontal and Vertical (boolean)
	...
	gw = lg.getWidth(), -- first Width captured to resize, do not works if you confugured(conf.lua) in fullscreen (or set gw)
	gh = lg.getHeight(), -- first Height captured to resize, do not works if you confugured(conf.lua) in fullscreen (or set gh)
	incenter = false, -- in center of screen (boolean)
	asize = false, -- auto size the listbox (boolean)
	font = lg.newFont(12), -- listbox font (object)
...
function listbox:calc(y)
	...
	if c > (self.max - 1) * #self.str / 100 and pos < self.h - 5 then
		...
		for i = 1, self.max do
			local id = self.min + i - 1
			self.tlist[i] = listbox:gettext(id)
		end
...
function listbox:setselected(i)
	if i < 1 or i > #self.str or not i then
		return false
	end
	self.selected = i
...
function listbox:addexpand(x, y)
	if self.expmode == "pos" then
		...
		if self.w > min and self.h > min then
			...
			lg.rectangle("fill", self.x + self.w, self.y + self.h, self.expx, self.expx, self.rsize, self.rsize)
		else
	...
	if self.expmode == "size" then
		...
		if (#self.tlist < self.max) then
			for i = 1, self.max do
				self.tlist[i] = listbox:gettext(i)
			end
		end
	end
end
...
function listbox:delitem(i)
	if not i or (i and i < 1 or i > #self.str) then
		return false
	end
	if #self.str <= 1 then
		listbox:delall()
...
function listbox:enudir(fpath, exts)
	...
	for fext in (exts .. " "):gmatch("([^%s]*)%s") do
		table.insert(ext, fext)
	end
	...
	for i, f in ipairs(itens) do
		if #ext > 0 and love.filesystem.isFile(fpath .. "/" .. f) then
			for j, e in pairs(ext) do
				if listbox:getfileext(f) == e then
...
function listbox:key(key, shortcut)
	if not self.selected or not self.enabled then
		return
	end
...
function listbox:drawtext(i, x, y)
	if (i > #self.str) or (self.min + i - 1 < 1) or self.tlist[i] == "" then
		return
	end
	local deffont = lg.getFont()
	...
	self.fwmaxshow =
		self.showindex and math.abs(self.fwmaxshow + self.fwmin - 1) or math.abs(self.fwmaxshow + self.fwmin - 1)

	tmpf =
		self.font:getWidth(tmpf) > (self.w - tmpwi) - self.paddingw and
		(self.fwmin < 2 and tmpf:sub(self.fwmin, self.fwmaxshow + 1) or tmpf:sub(self.fwmin, self.fwmaxshow)) or
		tmpf

	if
		(x > self.x and x < self.x + (self.w - self.radius * 1.1)) and (y > index and y < index + self.fh) and self.ispress and
			self.enabled and
			not listbox.ish and
			self.time > self.ptime
	 then
	...
	if self.selected == self.min + i - 1 then
		...
		lg.rectangle(self.selectedtip, self.x, self.imin, self.w, self.fh)
		lg.setColor(self.fselectedcolor)
	else
		lg.setColor(self.fcolor)
	end

	lg.setFont(self.font)
	lg.print(tmpi .. tmpf, self.x + self.paddingw, index)
	lg.setFont(deffont)
end
...
function listbox:draw()
	...
	if
		(self.scrypos + self.radius < self.y + self.h) and not listbox.isexpanding and not self.isscrlling and self.expand and
			x >= self.x + self.w and
			x <= self.x + self.w + self.expx + 1 and
			y >= self.y + self.h and
			y <= self.y + self.h + self.expx + 1 and
			self.ispress
	 then
		print(self.scrypos)
		self.asize = false
	...
	lg.setColor(self.bgcolor)
	lg.rectangle("fill", self.x, self.y, self.w, self.h, self.rsize, self.rsize)
	...
	if #self.str > 0 then
		...
		for i = 1, #self.tlist do
			listbox:drawtext(i, x, y, self.ispress)
		end
	end

	lg.setColor(self.bordercolor)
	lg.rectangle("line", self.x, self.y, self.w, self.h, self.rsize, self.rsize)

	if self.expand then
		lg.rectangle("line", self.x + self.w, self.y + self.h, self.expx, self.expx, self.rsize, self.rsize)
	end
	...
	if
		not listbox.isexpanding and (x > self.scrvx - self.radius and x < self.scrvx + self.scrvw) and
			(y > self.y and y < self.y + self.h) and
			self.ispress and
			self.max < #self.str and
			self.enabled
	 then
	...
	if not listbox.isexpanding and self.max < #self.str and self.ver then
		lg.circle("fill", self.scrvx, self.scrypos, self.radius, 100)
		lg.circle("line", self.scrvx, self.scrypos, self.radius, 100)
	end

	if
		not listbox.isexpanding and not self.isscrlling and (x > self.x and x < self.x + self.w - self.radius) and
			(y > self.y + self.h - self.radius * 1.1 and y < self.y + self.h + self.radius) and
			self.ispress and
			self.fwmaxshow > 0 and
			self.enabled
	 then
		self.scrhpos = x
		local pos = x - self.x
	...
	if not listbox.isexpanding and self.fwmin >= 2 and self.hor and self.maxsizew > self.w then
		lg.circle("fill", self.scrhpos, self.scrhy, self.radius, 100)
		lg.circle("line", self.scrhpos, self.scrhy, self.radius, 100)
	end
end
